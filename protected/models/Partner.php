...
class Partner extends CActiveRecord
{
	...
	public static function getAff ($good_id = FALSE,$email = '',$cupon = '')
	{
		...
		$check_relation_partner = Client::model ()->findbySQL ("SELECT * FROM {{client}} WHERE  email='".$email."' AND  relation_partner=1");
		if($check_relation_partner)
		{
			return $check_relation_partner->partner_id;		
		}
		
		// начальная проверка, если у купона есть партнер и стоит привязка
		if($cupon!='')
		{
			$relation_partner = Settings::item('relation_partner');
			if($relation_partner==1){
				//$check_relation = Client::model ()->findbySQL ("SELECT * FROM {{client}} WHERE  email='".$email."' AND  relation_partner=1");	
				$check_relation = Client::model ()->find('email=:email AND  relation_partner=1', array(':email'=>$email));
				if($check_relation)
				{
					if($check_relation->partner_id!=1)
					{
						return $check_relation->partner_id;		
					}
				}
				else{
					//$check_cupon = Cupon::model ()->findbySQL ("SELECT * FROM {{cupon}} WHERE  code='".$cupon."'");		
					$check_cupon = Cupon::model ()->find('code=:code', array(':code'=>$cupon));
					if($check_cupon->partner_id!=1)
					{
						return $check_cupon->partner_id;			
					}
				}		
			}
		}
		...
		// конец новой проверки
                ...
                if (Y::cookieGet ('ob_block_'.$good_id)==1) {
                    return '';
                }
            
		$aff = Y::cookieGet ('ob_affreg');
		...
		if (Settings::item('affShared')) {
			...
			$gaff = Y::cookieGet ('ob_ref_'.$good_id);
		} else {
			$gaff = '';
		}
 ...
 //Начисление партнёрского вознаграждения
        public static function doKomis ($o) {
			$bill_cupon = Bill::model()->findByPk ($o->bill_id);			
			
			//////////////////////////////////////////////////////////
			$relation_partner = Settings::item('relation_partner');
			
			if($relation_partner==1){
				$clients=Client::model()->findAll('email=:email', array(':email'=>$bill_cupon->email));	
				foreach ($clients as $client) { 
                   if($client->relation_partner==1)
					{
						$check_partner=$client->partner_id;						
					}
                }
			...
			if($relation_partner==1){
				if($bill_cupon->cupon!="")
				{
					//$check_cupon = Cupon::model ()->findbySQL ("SELECT * FROM {{cupon}} WHERE partner_id='".$refid."' AND good_id='".$o->good_id."' AND code='".$bill_cupon->cupon."'");
					
					$check_cupon = Cupon::model ()->findbySQL ("SELECT * FROM {{cupon}} WHERE good_id='".$o->good_id."' AND code='".$bill_cupon->cupon."'");		
					if($check_cupon)
					{	
						$c = Client::model()->find (
							'email = :email AND good_id = :id',
							array (
								':email' => $bill_cupon->email,
								':id' => $o->good_id,
							)
						);				
						if ($c) {
							$c->relation_partner = 1;
							$c->partner_id = $refid;
							$c->save (FALSE);									
						}	
					}
			...
			//echo "<pre>";print_r($bill_cupon);echo "</pre>";
			//print_r($bill_cupon->cupon);
			//print_r("DFDFDFDFDFDFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
			//проверяем есть ли у заказа Bill Cupon
			if($bill_cupon->cupon!="")
			{

				$admin_cupon = Cupon::model ()->findbySQL ("SELECT * FROM {{cupon}} WHERE good_id='".$o->good_id."' AND code='".$bill_cupon->cupon."'");				
				if($admin_cupon->partner_id==1)
				{					
					$sum_sale_partner=0;
				}
				else
				{				
					// здесь нужно от коммисии отнять скидку			
					$pull = Cupon::model ()->findbySQL ("SELECT * FROM {{cupon}} WHERE partner_id='".$check_cupon->partner_id."' AND good_id='".$o->good_id."' AND code='".$bill_cupon->cupon."'");	
					
					//echo "<pre>";print_r($p);echo "</pre>";
					//print_r($pull->kind_id);
					if($pull->kind_id=="fixed"){
						$sum_sale_partner = $pull->sum;
						//echo "СКИДКА".$pull->sum;
					}
					else{	
						$sum_sale_partner = round ($g->price*($pull->sum)/100,2);
						/*echo "Цена".$g->price;
						echo "ФИКС".$pull->sum;
						echo "КОМИССИЯ".$komis;*/
					}	
			...
			/*print_r($sum_sale_partner);
			print_r($sum);*/
			$sum=($sum)-($sum_sale_partner);
