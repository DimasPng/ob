...
class NwaysPatch
{
    public function actionPatch ($silent = true)
    {
       ...
       /* $this->_create_setting('payYandexOn','Включить Я.Деньги',0);
        $this->_create_setting('payYandexAccount','№ счёта Я.Деньги',"");
        $this->_create_setting('payYandexKey','Секретное слово Я.Деньги',"");

        //Форма способа Яндекс.Деньги
        $way = Way::model ()->find ('way_id = "yandex_online"');
        if (!$way)
        {
            $this->_import_way ("yandex_online","Яндекс.Деньги (через сайт)");
        } else {
            echo "Способ Яндекс.Деньги уже добавлен ранее\r\n";
        }

        //Яндекс.Деньги для раздела "Выбор оплаты"
        $this->_one_waylist('yandex_online',"Оплата Яндекс.Деньги или VISA/MasterCard",'yandex','Электронные платежи',180,'http://money.yandex.ru/');
        ...
        $this->_one_waylist('paypal_online',"Оплата через PayPal",'paypal','Электронные платежи',182,'http://paypal.com/');
        ...
        $this->_one_waylist('qiwi_online',"Оплата с помощью Qiwi",'qiwi','Электронные платежи',184,'http://qiwi.ru/');
        ...
        //Qiwi для раздела "Выбор оплаты"
        $this->_one_waylist('w1_online',"Оплата через Единую Кассу (W1)",'w1','Электронные платежи',186,'http://w1.ru/');
