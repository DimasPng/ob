...
class Good extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, category_id, title, price, currency, kind, affOn,  affShow, used, mli', 'required'),
			...
			array('category_id, catalog_on, position, affOn, affShow, used, securebook, cartOn, upsellOn, csellOn, authorKomis, authorKomis2, authorKomis3', 'numerical', 'integerOnly'=>true),
			array('price, affKomis, affOrder, affPkomis, cartMinus, mlgroup, sendpulseGroup', 'numerical'),
			array('id, needid, sendid, author_id, author_id2, author_id3', 'length', 'max'=>100),
	...
	public function attributeLabels()
	{
		return array(
			...
			'affPkomisType' => 'Тип комиссии 2 уровня',
                        ...
                        'authorKomis2' => 'Авторский процент',
                        'authorKomis3' => 'Авторский процент',
			'affShow' => 'Показывать партнёру',
			'used' => 'Товар активен',
			'mli' => 'Включить интеграцию с Mailer Lite',
			'mlgroup' => 'Введите код группы Mailer Lite',
            'sendpulseGroup' => 'Выберите группу для сохранения SendPulse',
			...
			'cartText' => 'Текст для корзины',
                        ...
                        'aukind2' => 'Тип % для автора',
                        'aukind3' => 'Тип % для автора',
	...
	public function search()
	{
		...
		//Функция возвращает список всех товаров у которых включена выплата коммисионных
        public static function items_aff ($empty = FALSE) {
            ...
            foreach ($goods as $one) {
				if($one->affOn==1  && $one->affShow){
					if($one->affKomisType=="fixed")
					{
						$ng[$one->id] = "(".$one->affKomis."руб.) ".$one->title;
					}
					else
					{
						$ng[$one->id] = "(".$one->affKomis."%) ".$one->title;
					}
				}
