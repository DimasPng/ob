...
class ClickController extends Controller
{
	public function actionGo($refid = FALSE, $good_id = FALSE, $page = 'a', $channel = 'default')
	{
		...
		if (Settings::item ('affLast')!=1) {
			...
			$cookie = Y::cookieGet ('ob_affreg');
			if (empty($cookie)) {
				//Общий кукиес
				Y::cookieSet ('ob_affreg',$refid);
			}

			$cookie = Y::cookieGet ('ob_ref_'.$good_id);
			if (empty($cookie)) {
				//Общий кукиес
				Y::cookieSet ('ob_ref_'.$good_id,$refid);
			}
			...
			Y::cookieSet ('ob_ref_'.$good_id,$refid);
                ...
                Y::cookieSet ('ob_channel',$channel); //Канал
