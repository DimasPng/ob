...
class Author extends CActiveRecord
{
	...
	private static function loadItems()
	{
        ...
        public static function doKomis ($o) {
            ...
            /******************************************************************/         
            //Есть ли авторское вознаграждение?
            if ($g->authorKomis2<=0) return FALSE;            
            
            //Проверяем есть ли автор - мало ли?
            if (!$g->author_id2) return FALSE;
            
            //Загружаем автора:
            $a = Author::model()->findByPk ($g->author_id2);
            
            if (!$a) return FALSE;            
            //Подсчитываем процент            
            $nncena = $o->cena;
            //Иначе процент от комиссии
            if (($g->aukind2 == 'total') AND (!empty ($o->partner_id))) {
                                    //Находим запись комиссионных
                    $xx = Affstats::model ()->findByPk($o->id);
                    $comis = $xx->komis + $xx->pkomis;
                    $osum = Valuta::conv ($o->cena,$o->valuta);
                    ...
                    $ncena = round ($nncena*$g->authorKomis2/100,2);
                    $vv = array ('rur' => $ncena); //Так как уже в рублях
            ...
            //Есть ли авторское вознаграждение?
            if ($g->authorKomis3<=0) return FALSE;            
            
            //Проверяем есть ли автор - мало ли?
            if (!$g->author_id3) return FALSE;
            
            //Загружаем автора:
            $a = Author::model()->findByPk ($g->author_id3);
            
            if (!$a) return FALSE;            
            //Подсчитываем процент            
            $nncena = $o->cena;
            //Иначе процент от комиссии
            if (($g->aukind3 == 'total') AND (!empty ($o->partner_id))) {
                                    //Находим запись комиссионных
                    $xx = Affstats::model ()->findByPk($o->id);
                    $comis = $xx->komis + $xx->pkomis;
                    $osum = Valuta::conv ($o->cena,$o->valuta);
                    ...
                    $ncena = round ($nncena*$g->authorKomis3/100,2);
                    $vv = array ('rur' => $ncena); //Так как уже в рублях
