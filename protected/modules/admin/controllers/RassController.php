...
class RassController extends Controller
{
	...
	public function actionMsg($bills = FALSE)
	{
                ...
                if ($bills != FALSE) {
					...
					$counter = 0;
					$delete_item;
					// счетчик для проверки и удалиения активных или не активных
					$counter_activity=0;
					
					$day_click = Settings::item('day_click');
					$day_click_unix= time() - ($day_click*86400);
					$count_click = Settings::item('count_click');
                    ...
                    if ($_POST['type']=="refs") {
					...
					}	elseif ($_POST['type']=="no_active_partners") {
						...
						foreach ($u as $u_mas) {
							 if($type=="refs"){
								$m_click = Mailclick::model()->findAll("ref_id='".$u_mas->id."' AND date_click>".$day_click_unix);
							 }
							 else{
								$m_click = Mailclick::model()->findAll("client_id='".$u_mas->id."' AND date_click>".$day_click_unix);								
							 }

							 $count_click_diaposon = count ($m_click);

							 if($count_click_diaposon<$count_click)
							 {
								
							 }
							 else
							 {
								unset($u[$counter_activity]);	
							 }
							 $counter_activity++;							 
						 }
					
					
					 }	elseif ($_POST['type']=="no_active_clients") {
					...
					//этот пункт по товарам идет проверка на клиентов
					 }else {              
						$list_ids = '';
					...
					// здесь мы будем проверять в таблице Mailclick активность пользователей в соответствии с настройками
	...
	public function actionSend()
	{
            ...
            foreach ($users as $u) {
				...
				if($d['type']=="partner"){
					$d['mail_click'] = Y::bu().'mailclick?email='. $d['email'].'&type='.$d['type'].'&ref_id='.$d['refid'].'&redirect='.$link;
				}
				else{
					$d['mail_click'] = Y::bu().'mailclick?email='. $d['email'].'&type='.$d['type'].'&client_id='.$d['refid'].'&redirect='.$link;
				}
