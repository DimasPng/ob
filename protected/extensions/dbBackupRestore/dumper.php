...
class dumper {
	...
	protected function init()
		...
		$this->size = 0;
		$this->comp = 0;
		$this->links = @mysqli_connect($this->dbhost, $this->dbuser, $this->dbpass);
                if ($this->links){
                	$auth = 1;
                }
                else{
		...
		$this->mysql_version = mysqli_get_server_version($this->links);
	...
	public function backup() {
		...
		echo tpl_l("Подключение к БД `{$db}`.");
        	if ($this->links = @mysqli_connect($this->dbhost, $this->dbuser, $this->dbpass)){
        		$auth = 1;
        	}
        	else{
        		$error = '#' . mysqli_connect_errno() . ': ' . mysqli_connect_error();
		...
		mysqli_select_db($this->links, $db) or trigger_error ("Не удается выбрать базу данных.<BR>" . mysqli_error($this->links), E_USER_ERROR);
		$tables = array();
                $result = mysqli_query($this->links, "SHOW TABLES");
		$all = 0;
		...
		$result = mysqli_query($this->links, "SHOW TABLE STATUS");
		$tabinfo = array();
		$tab_charset = array();
		$tab_type = array();
		$tabinfo[0] = 0;
		...
		$result = mysqli_query($this->links, "SET SQL_QUOTE_SHOW_CREATE = 1");
		// Кодировка соединения по умолчанию
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysqli_query($this->links, "SET NAMES '" . CHARSET . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error($this->links), E_USER_ERROR);
        ...
        foreach ($tables AS $table){
			...
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					mysqli_query($this->links, "SET NAMES '" . $tab_charset[$table] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error($this->links), E_USER_ERROR);
        	...
        	// Создание таблицы
			$result = mysqli_query($this->links, "SHOW CREATE TABLE `{$table}`");
            ...
            $NumericColumn = array();
            $result = mysqli_query($this->links, "SHOW COLUMNS FROM `{$table}`");
            $field = 0;
            ...
            while(($result = mysqli_query($this->links, "SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) && ($total = mysqli_num_rows($result))){
            		while($row = mysqli_fetch_row($result)) {
						...
						for($k = 0; $k < $fields; $k++){
                    		if ($NumericColumn[$k])
                    			...
                    			$row[$k] = isset($row[$k]) ? "'" . mysqli_real_escape_string ($this->links, $row[$k]) . "'" : "NULL";
                    	}
	...
	public function restore(){
		...
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
			mysqli_query( $this->links,"SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error($this->links), E_USER_ERROR);
			...
			if ($sql) {
			    if (preg_match("/;$/", $str)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										mysqli_query($this->links, "SET NAMES '" . $charset[2] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error($this->links), E_USER_ERROR);
							...
							elseif(CHARSET == 'auto'){ // Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
								...
								if ($this->restore_charset != $last_charset) {
									mysqli_query($this->links, "SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error($this->links), E_USER_ERROR);
					...
					elseif($this->mysql_version > 40101 && empty($last_charset)) { // Устанавливаем кодировку на случай если отсутствует CREATE TABLE
						mysqli_query($this->links, "SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysqli_error($this->links), E_USER_ERROR);
						echo tpl_l("Установлена кодировка соединения `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
            		$insert = '';
    			...
    			if ($execute) {
            		$q++;
            		mysqli_query($this->links, $sql) or trigger_error ("Неправильный запрос.<BR>" . mysqli_error($this->links), E_USER_ERROR);
					if (preg_match("/^insert/i", $sql)) {
            		    $aff_rows += mysqli_affected_rows($this->links);
            		}
            		$sql = '';
            		$query_len = 0;
            		$execute = 0;
	...
	function db_select(){
		if ($this->dbnames != '') {
			...
			foreach($items AS $item){
    			if (mysqli_select_db($this->links, $item)) {
    				$tables = mysqli_query($this->links, "SHOW TABLES");
    				if ($tables) {
    		...
    		$result = mysqli_query($this->links, "SHOW DATABASES");
    		$dbs = array();
    		while($item = mysqli_fetch_array($result)){
    			if (mysqli_select_db($this->links, $item[0])) {
    				$tables = mysqli_query($this->links, "SHOW TABLES");
    				if ($tables) {
    	  			    $tabs = mysqli_num_rows($tables);
